...
import { S_R3_MAX_POWER } from '../../config/CSamurai';

export const SamuraisRealm = () => {
    ...
    useEffect(() => {
        axios.get("http://localhost:8080/warriors/rank/swordsman")
            .then((res) => {setAllR1(res.data)})   
            .catch((err) => {console.log(err)}) 
        axios.get("http://localhost:8080/warriors/rank/samurai")
            .then((res) => {setAllR2(res.data)}) 
            .catch((err) => {console.log(err)}) 
        axios.get("http://localhost:8080/warriors/rank/shogun")
    ...
    const updateWarrior = (warrior:IWarrior) => {
        ...
        if(warrior.power === S_R3_MAX_POWER){           
            SetPopupMAX(true)
            setTimeout(() => {
                SetPopupMAX(false)
            }, 500)
                ...
                axios.put("http://localhost:8080/warriors/" + warrior._id, warriorUpgrade(warrior))
                    .then((res) => {
                        ...
                        if(res.data.rank === temp){
                            setAllR1(allR1.filter((war:IWarrior) => {return war.rank === "swordsman"}))
                            setAllR2(allR2.filter((war:IWarrior) => {return war.rank === "samurai"}))
                            setAllR3(allR3.filter((war:IWarrior) => {return war.rank === "shogun"}))
                        } else
                        if(temp === "swordsman" && res.data.rank === "samurai"){
                            setAllR1(allR1.filter((war:IWarrior) => {return war.rank === "swordsman"}))
                            setAllR2(allR2.concat(res.data))
                        } else 
                        if(temp === "samurai" && res.data.rank === "shogun"){
                            setAllR2(allR2.filter((war:IWarrior) => {return war.rank === "samurai"}))
    ...
    const sectionR1switch:JSX.Element = <>
        ...
        <section    className="w-full flex justify-between items-center">
            ...
            <button className={`uppercase font-extrabold text-slate-800 ${showR1 ? `underline` : ""} hover:opacity-70`} onClick={() => {showR1 ? setShowR1(false) : setShowR1(true)}}>swordsmans</button>
            <div    className="w-2/5 h-1 bg-slate-800 rounded-md"></div>
        </section>
    ...
    const sectionR1:JSX.Element = <>
        ...
        <div className='w-full  warrior-item flex flex-wrap justify-center'>
            <div className="recruit w-28 h-44 m-1 p-0 bg-rose-400 rounded-md shadow-lg flex flex-col justify-center items-center cursor-pointer" onClick={() => {createWarrior("samurais")}}>
                <div className="w-24 h-24 bg-swordsman-e bg-cover"></div>
            ...
            {allR1.map((warrior:IWarrior):JSX.Element => {
                return (
                    <div className="warrior w-28 h-44 m-1 p-0 bg-rose-400 rounded-md shadow-lg flex flex-col justify-center items-center cursor-pointer" key={warrior._id} onClick={() => {
    ...
    const sectionR2switch:JSX.Element = <>
        ...
        <section    className="w-full flex justify-between items-center">
            <div    className="w-2/5 h-1 bg-slate-800 rounded-md"></div>
            <button className={`uppercase font-extrabold text-slate-800 ${showR2 ? `underline` : ""} hover:opacity-70`} onClick={() => {showR2 ? setShowR2(false) : setShowR2(true)}}>samurais</button>
            <div    className="w-2/5 h-1 bg-slate-800 rounded-md"></div>
        </section>
    ...
    const sectionR3switch:JSX.Element = <>
        ...
        <section    className="w-full flex justify-between items-center">
            <div    className="w-2/5 h-1 bg-slate-800 rounded-md"></div>
            <button className={`uppercase font-extrabold text-slate-800 ${showR3 ? `underline` : ""} hover:opacity-70`} onClick={() => {showR3 ? setShowR3(false) : setShowR3(true)}}>shoguns</button>
            <div    className="w-2/5 h-1 bg-slate-800 rounded-md"></div>
        </section>
    ...
    return (
    <div className="w-full min-h-screen m-0 p-3 bg-rose-500 flex flex-col ">
        <Header />
            {popupLVL ? <PopupLVL /> : <></>}
            {popupRIP ? <PopupRIP /> : <></>}
            {popupMAX ? <PopupMAX /> : <></>}
